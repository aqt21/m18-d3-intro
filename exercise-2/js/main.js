...
$(function() {
  ...
  var myPeople = myDiv.selectAll("p").data(people, function(d){return d.name});
  ...
  setTimeout(function(){
    ...
    // Make sure to specify the `name` attribute as an identifying characteristic in your data-join.
    var newPeople = myDiv.selectAll("p").data(people2, function(d){return d.name});

    // Determine which people are no longer represented in the data (`exit`), and transition their opacity to 0 and remove them.

    newPeople.exit().transition().style("opacity","0").remove();
  }, 3000);
});
